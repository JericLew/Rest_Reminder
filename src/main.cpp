...
const unsigned long ALERT_TRIGGER = 3600000; 
const unsigned long ALERT_INTERVAL = 1000;
const float TEMP_DIFF = 0.6;
...
bool thermalCheck() {
  if (mlx.readObjectTempC() - mlx.readAmbientTempC() > TEMP_DIFF) {
    return true;
  }
  return false;
}
...
void lcdPrintStatus(bool thermal_status, bool tracking_status, bool alert_status){
  ...
  if (thermal_status & tracking_status & not alert_status) {
    lcd.setCursor(0,1); lcd.print("Detected t:"); lcd.print((millis()-starttime)/1000/60); lcd.print("m"); lcd.print((millis()-starttime)%60000/1000);lcd.print("s");
  }
  if (thermal_status & tracking_status & alert_status) {
    lcd.setCursor(0,1); lcd.print("Alert!   t:"); lcd.print( (millis()-starttime)/1000); lcd.print("m"); lcd.print((millis()-starttime)%60000/1000);lcd.print("s");
  }

  if (not thermal_status & tracking_status) {
    lcd.setCursor(0,1); lcd.print("Waiting  t:"); lcd.print( (millis()-starttime)/1000/60); lcd.print("m"); lcd.print((millis()-starttime)%60000/1000);lcd.print("s");
  }
